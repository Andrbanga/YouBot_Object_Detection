...
vc = cv2.VideoCapture("http://192.168.88.22:8080/stream?topic=/camera/rgb/image_raw&width=640&height=480&quality=30")
...
def getNearestPoint(points):
    nearestDist = 100000
    nearestPoint = points[1]
    dist = getDistance(points)
    if dist < nearestDist:
        nearestDist = dist
        nearestPoint = points
        return nearestPoint
...
def roundTuple(items):  #sorry, I know I must use normal vectors
        ...
        for i in range(min(max_boxes_to_draw, boxes.shape[0])):
            if scores is None or scores[i] > .7 and classes[i] == 1:
                ...
                if box is not None:
                    points = box
                    if len(points) > 0:
                        nearestPoint = getNearestPoint(points)
                        #nearestPoint = points
                        drawRect(nearestPoint)
                        nearestPoint = ((nearestPoint[1] + nearestPoint[3]) / 2, nearestPoint[0] + 30)
                        cv2.line(frame, roundTuple(midpoint), roundTuple(nearestPoint), (232, 244, 66), 2)
                        # approx
                        elapsedTimeSinceLastCommand = time.time()
                        lastCommandTime = time.time()

                        distance = sqrt(getDistance(nearestPoint))
